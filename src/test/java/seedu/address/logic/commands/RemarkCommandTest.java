...
class RemarkCommandTest {
    ...
    private ModelPerson model = new ModelPersonManager(getTypicalAddressBook(), new UserPrefs());
    ...
    void execute_addRemarkUnfilteredList_success() {
        ...
        String expectedMessage = String.format(RemarkCommand.MESSAGE_REMARK_PERSON_SUCCESS,
                INDEX_FIRST_PERSON.getOneBased(), REMARK_STUB);

        RemarkCommand remarkCommand = new RemarkCommand(INDEX_FIRST_PERSON, new Remark(editedPerson.getRemark().value));

        ModelPerson expectedModel = new ModelPersonManager(new AddressBook(model.getAddressBook()), new UserPrefs());
    ...
    public void execute_deleteRemarkUnfilteredList_success() {
        Person firstPerson = model.getFilteredPersonList().get(INDEX_FIRST_PERSON.getZeroBased());
        Person editedPerson = new PersonBuilder(firstPerson).withRemark(REMARK_EMPTY).build();

        RemarkCommand remarkCommand = new RemarkCommand(INDEX_FIRST_PERSON,
                new Remark(editedPerson.getRemark().toString()));

        String expectedMessage = String.format(RemarkCommand.MESSAGE_REMARK_PERSON_SUCCESS,
                INDEX_FIRST_PERSON.getOneBased(), REMARK_EMPTY);
